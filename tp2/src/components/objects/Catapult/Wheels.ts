import {Color, CylinderGeometry, Group, Mesh, MeshBasicMaterial, MeshPhongMaterial, Texture} from "three";

const createWheel = (height: number, texture: Texture): Group => {
    const colorGreaterWheel = 0x674e37;
    const geometry = new CylinderGeometry( .5, .5, height );
    const material = new MeshPhongMaterial( { color: colorGreaterWheel, map: texture } );
